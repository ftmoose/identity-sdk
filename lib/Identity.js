...
Identity.loginUser = async function loginUser({ username, email, password }) {
  ...
  // Generate tokens
  const accessToken = await JwtManager.generateToken('access', user.toObject());
  const refreshToken = await JwtManager.generateToken('refresh', user.toObject());
